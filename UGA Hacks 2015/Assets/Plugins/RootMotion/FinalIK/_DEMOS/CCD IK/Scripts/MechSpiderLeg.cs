...
namespace RootMotion.Demos {
	...
	public class MechSpiderLeg : MonoBehaviour {
		...
		private Vector3 GetStepTarget(out bool stepFound, float focus, float distance) {
			...
			if (Physics.Raycast(stepTarget + up * mechSpider.raycastHeight, -up, out hit, mechSpider.raycastHeight + distance, mechSpider.raycastLayers)) stepFound = true;
		...
		void Update () {
			...
			Vector3 idealPosition = GetStepTarget(out stepFound, raycastFocus, mechSpider.raycastDistance);
			if (!stepFound) idealPosition = GetStepTarget(out stepFound, -raycastFocus, mechSpider.raycastDistance * 3f); // Try again with inverted focus
			if (!stepFound) return;
